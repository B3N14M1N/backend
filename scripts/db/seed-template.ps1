...
& $DC 'exec' $dbC 'psql' '-U' $PGUSER '-d' $PGDB '-c' "DROP TABLE IF EXISTS templateitem; DROP TYPE IF EXISTS templatestatus; CREATE TYPE templatestatus AS ENUM ('DRAFT', 'PUBLISHED'); CREATE TABLE templateitem (id uuid primary key DEFAULT gen_random_uuid(), title varchar(200) NOT NULL, body text, status templatestatus NOT NULL DEFAULT 'DRAFT', created_at timestamptz DEFAULT now(), updated_at timestamptz DEFAULT now());"
